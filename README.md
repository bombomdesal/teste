...
Medidas de Desempenho e de Riscos:
 Volatilidade;
 Value At Risk;
...
armazenamento de dados:
 ...
 principais estruturas de armazenamento de dados analíticos (data 
warehouse, data mart, data lake data lakehouse, vector stores), suas diferenças conceituais e casos de uso;
 armazenamento na 
nuvem.
 4.
...
Modelo de dados:
 ...
 modelo 
relacional:
 tabelas, esquemas, chaves, consultas;
...
modelo colunar;
 ...
 definição de 
ingestão em lote (batch) e em tempo real (stream).
 7.
 Big Data:
 conceito de big data;
 conceitos gerais sobre técnicas e ferramentas 
para lidar com grandes volumes de dados (Spark, Hadoop, HDFS e MapReduce).
 V - GESTÃO DE PROJETOS DE CIÊNCIA DE 
DADOS:
 1.
 Ciclo de vida de projetos de ciência de dados.
 2.
 Metodologias de gestão de projetos de ciência de dados:
 CRISP-DM;
...
Principais papéis envolvidos em projetos de ciência de dados.
...
importância para avaliação da qualidade de dados;
 ...
 2.
 Coleta de dados:
 fontes comuns de dados (internas e 
externas);
...
padronização;
 ...
 Feature engineering:
 processos para 
enriquecimento de dados, com criação e seleção de features relevantes;
 transformações matemáticas e estatísticas comuns em 
variáveis.
 7.
 Divisão de dados:
 técnicas de amostragem;
 divisão entre treinamento, validação e teste;
 abordagens para cross-
validation.
 VII – MODELAGEM:
 1.
...
random search;
 ...
 3.
 Métricas para avaliação e 
seleção de modelos:
 métricas para regressão (MSE;
 RMSE;
 MAE;
 R²;
 R² ajustado);
 métricas para classificação (accuracy, precision, 
recall, F1-score e ROC-AUC);
...
Técnicas de regularização:
 ...
 técnicas 
para lidar com dados desbalanceados;
 oversampling;
 undersampling;
 dados sintéticos;
 ajuste de pesos.
...
logística;
 ...
 Decision Trees (CART);
 classificadores Naive-Bayes (Binomial-
Beta, Poisson-Gama, Normal-Normal);
 Florestas Aleatórias (Random Forest).
 4.
 Introdução à regressão:
 regressão linear simples e 
múltipla;
 hipóteses clássicas, método dos mínimos quadrados, diagnóstico e avaliação de modelos de regressão (F-test, coeficiente 
de determinação, análise de resíduos e demais), testes de significância, intervalos de confiança, análise ANOVA, modelos não lineares 
(log-log, lin-log, log-lin e inverso).
 5.
...
com variáveis temporais (ARIMAX).
 8.
...
estatística, experimentos e quase-experimentos, desenho de descontinuidade de regressão, modelos de variáveis instrumentais, 
diferenças em diferenças, modelos de equações estruturais (SEM), métodos de pareamento.
10.
 Redes neurais:
 Introdução a Redes 
Neurais Artificiais (arquitetura, funções de ativação, treinamento, forward pass, backpropagation, loss functions, algoritmos de 
otimização, épocas, batch size e demais);
 embeddings;
...
imagens.
 ...
 14.
 Quantificação de incertezas em modelos preditivos:
 Programação 
Probabilística;
 Amostragem de Gibbs;
...
N-grams;
 CBoW;
 FTD-IDF;
 word embeddings (Word2Vec, GloVe e demais) e document embeddings (Doc2Vec, BERT, ELMo e 
demais).
 3.
...
limpeza;
 transformação e pré-processamento de dado);
...
(Structured Query Language):
 ...
 comandos básicos para consultas (inserção, atualização e exclusão de dados) 
e para análise de dados (como funções de agregação, filtros, joins, subconsultas e demais).
 4.
 Gestão de Código:
 qualidade de código;
...
testes automatizados;
 ...
 linha de comando 
(navegação em diretórios, manipulação de arquivos e dados);
...
visualizações e gráficos:
 ...
 principais conceitos de codificação 
visual;
 interatividade;
...
acessibilidade.
 ...
 componentes de um storytelling 
efetivo.
 5.
 Reportes executivos:
 princípios de comunicação corporativa;
 interpretação e apresentação de dados de resultados de 
análises e de insights.
 XII – GOVERNANÇA E SEGURANÇA DE DADOS:
 1.
 Noções de governança de dados (DMBOK):
 conceitos 
e objetivos da governança de dados;
 principais técnicas de qualidade e integridade de dados;
 princípios de privacidade e proteção a 
dados.
 XIII – GOVERNANÇA, SEGURANÇA E APLICAÇÃO RESPONSÁVEL DE IA:
 1.
 Noções de governança de IA:
 conceitos e 
objetivos da governança de IA;
